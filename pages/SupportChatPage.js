...
const supportAvatar = require('../img/ic_sirius_logo_small.png');

class SupportChatPage extends React.Component {
  ...
  componentDidMount() {
    ...
    getDialogMessages(Const.SUPPORT_SKYTAG, 100, -1, 'backward', (pr, packet) => {
      this.setState({ isProgress: false });
      const { data } = packet;
      if (0 === data.result) {
  ...
  sendTextMessage() {
    ...
    sendMessageToUser(Const.SUPPORT_SKYTAG, message, function(pr, packet) {
  ...
  renderItem(item, photoUrl) {
    ...
    return !data.myOwn ? (
      <ChatBubbleTextTheir text={data.text} ts={ts} localImage={supportAvatar} />
    ) : (
