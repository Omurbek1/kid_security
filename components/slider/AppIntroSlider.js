...
export default class AppIntroSlider extends React.Component {
  ...
  goToSlide = (pageNum) => {
    if (Platform.OS === 'ios' && this.state.animation) {
      return;
    }
    this.setState({ activeIndex: pageNum, animation: true });
  ...
  _renderDefaultButton = (name) => {
    let content = (
      <Text style={[styles.buttonText, this.props.buttonTextStyle]}>{this.props[`${name.toLowerCase()}Label`]}</Text>
    );
  ...
  _renderPagination = () => {
    ...
    return (
      <SafeAreaView style={[styles.paginationContainer, this.props.paginationStyle]}>
  ...
  _onMomentumScrollEnd = (e) => {
    ...
    this.setState({ activeIndex: newIndex, animation: false });
